<template>
  ...
  <CollapseItem name="地图" :expanded="true">
    <SettingItemBox name="地图区域">
      ...
      <SettingItem name="所属市" v-if="mapRegion.adcode == 330000">
        <n-select
          size="small"
          v-model:value="mapRegion.municipalityAdcode"
          :options="mapMunicipalityRegionOptions"
          value-field="adcode"
          label-field="name"
        />
      </SettingItem>
...
const mapMunicipalityRegionOptions = ref()
...
watch(
  () => {
    mapRegion.value.adcode
  },
  async newVal => {
    // console.log(mapRegion.value.adcode)
    if (mapRegion.value.adcode == 330000) {
      mapMunicipalityRegionOptions.value = await new Promise<boolean>(resolve => {
        import(`./mapGeojson/${mapRegion.value.adcode}.json`).then(async data => {
          console.log(data)
          let tem: any = [
            {
              adcode: mapRegion.value.adcode,
              name: '全省'
            }
          ]
          await data.features.forEach((element: any) => {
            if (element.properties.name) {
              tem.push({ ...element.properties })
            }
          })
          resolve(tem)
        })
      ...
      mapMunicipalityRegionOptions.value = []
    }
  },
  {
    immediate: true,
