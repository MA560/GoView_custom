...
// 两种模式 单点登录 和 login直接登录
// ---单点登录---
const route = useRoute();
const { query } = route;
const { token = '', userName = '', fromPath = '' } = query as { token: string, userName: string, fromPath: string };
if (query && token) {
  const { nickname, username, id }: { nickname: string, username: string, id: string } = {
    nickname: userName,
    username: userName,
    id: "5"
  };
  // 携带的 token 格式
  const { tokenValue, tokenName }: { tokenValue: string, tokenName: string } = {
  ...
  systemStore.setItem(SystemStoreEnum.USER_INFO, {
    ...
    [SystemStoreUserInfoEnum.FROM_PATH]: fromPath,
    t
  })
...
const handleSubmit = async (e: Event) => {
  ...
  formRef.value.validate(async (errors: any) => {
    if (!errors) {
      ...
      if (res?.code == 200) {
        const tokenData: { tokenValue: string, tokenName: string } = {
          tokenValue: 'Bearer ' + res.token as string,
          tokenName: 'Authorization',
        };
        let userinfo: { nickname: string, username: string, id: string } = {
          nickname: getName,
          username: getName,
          id: "5"
        };

        const { nickname, username, id }: { nickname: string, username: string, id: string } = userinfo;
        const { tokenValue, tokenName }: { tokenValue: string, tokenName: string } = tokenData;
        ...
        loading.value = false
        window['$message'].error(t('login.login_error'))
        return
      }
      loading.value = false
    } else {
