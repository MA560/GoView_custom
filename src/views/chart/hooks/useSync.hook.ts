...
// 接口
import { saveProjectApi, fetchProjectApi, uploadFile, updateProjectApi, updateDataScreen } from '@/api/path'
// 画布枚举
...
export const useSync = () => {
  ...
  const updateStoreInfo = (projectData: {
    ...
    const { id, name, remarks, imageUrl, isPublish } = projectData
    // ID
    ...
    // 缩略图
    chartEditStore.setProjectInfo(ProjectInfoEnum.THUMBNAIL, imageUrl)
    // 发布
    chartEditStore.setProjectInfo(ProjectInfoEnum.RELEASE, isPublish === 1)
  }
    ...
    try {
      ...
      if (res && res.code === ResultEnum.SUCCESS) {
        if (res.data) {
          ...
          // 更新全局数据
          if(res.data.dataJson) await updateComponent(JSONParse(res.data.dataJson))
          return
        } else {
  ...
  const dataSyncUpdate = throttle(async (updateImg = true) => {
    ...
    let params: Params = {
      ...
      // name: "测试",
      imageUrl: imageUrl,
      dataJson: JSONStringify(chartEditStore.getStorageInfo || {}),
    };
    if (projectId) {
      params.id = projectId;
    }
    // 创建和保存项目
    const res = await updateDataScreen(params);
